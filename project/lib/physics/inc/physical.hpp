...
struct State {
    ...
    explicit State(const double mass);
    ~State() = default;
...
public:
    ...
    void setInteractivity(const bool interactive) { interactive_ = interactive; }
    bool hasGeometry() const { return static_cast<bool>(geometry_); }
